...
public final class FloatingControlStore {
    ...
    public static void position(Context c,String id,int x,int y){Item old=get(c,id);if(old!=null)save(c,new Item(old.id,old.name,old.host,x,y));}
    ...
    private static void write(Context c,List<Item> items){JSONArray a=new JSONArray();try{for(Item item:items){JSONObject o=new JSONObject();o.put("id",item.id);o.put("name",item.name);o.put("host",item.host);o.put("x",item.x);o.put("y",item.y);a.put(o);}}catch(Exception ignored){}prefs(c).edit().putString(KEY,a.toString()).apply();}
