...
public final class WidgetPrefs {
 ...
 public static int backgroundOpacity(Context c,int id){return p(c).getInt("bo"+id,opacity(c,id));}
 public static void setBackgroundOpacity(Context c,int id,int opacity){p(c).edit().putInt("bo"+id,Math.max(0,Math.min(100,opacity))).apply();}
 ...
 public static void remove(Context c,int id){android.content.SharedPreferences.Editor e=p(c).edit().remove("h"+id).remove("r"+id).remove("c"+id).remove("k"+id).remove("m"+id).remove("i"+id).remove("o"+id).remove("bo"+id).remove("fv"+id).remove("fmin"+id).remove("fmax"+id).remove("fweb"+id).remove("flan"+id);for(int i=0;i<4;i++)e.remove("fvar"+id+"_"+i);e.apply();}
}
