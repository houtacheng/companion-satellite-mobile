...
public class MainActivity extends Activity {
    ...
    private void showControls() {
        ...
        labels.add("＋ 新增控制項");labels.add("＋ 加入通知欄下拉面板");labels.add("Satellite 連線診斷");
        new AlertDialog.Builder(this).setTitle("Companion 控制項").setItems(labels.toArray(new String[0]),(d,w)->{if(w<slots.size())editControl(slots.get(w));else if(w==slots.size()){int slot=ControlStore.firstAvailable(this);if(slot<0)Toast.makeText(this,"最多可建立 12 個控制項",Toast.LENGTH_LONG).show();else editControl(slot);}else if(w==slots.size()+1)chooseTile();else showSatelliteDiagnostics();}).setNegativeButton("取消",null).show();
    }
