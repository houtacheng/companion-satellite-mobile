...
public class MainActivity extends Activity {
    ...
    private void showFloatingControls(){
        List<FloatingControlStore.Item> items=FloatingControlStore.all(this);ArrayList<String> labels=new ArrayList<>();for(FloatingControlStore.Item item:items)labels.add("●  "+item.name);labels.add("＋ 新增懸浮旋鈕");labels.add("◎ 設定 Companion App 啟動球");if(!items.isEmpty())labels.add(FloatingControlStore.enabled(this)?"暫停所有懸浮旋鈕":"啟動所有懸浮旋鈕");
        new AlertDialog.Builder(this).setTitle("懸浮工具 · 長按可移動").setItems(labels.toArray(new String[0]),(d,w)->{if(w<items.size())floatingActions(items.get(w));else if(w==items.size())editFloating(null);else if(w==items.size()+1)configureLauncherBall();else{boolean enable=!FloatingControlStore.enabled(this);FloatingControlStore.setEnabled(this,enable);if(enable)ensureOverlayAndStart();else FloatingControlService.stopAll(this);}}).setNegativeButton("取消",null).show();
    }
